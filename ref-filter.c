...
static int match_name_as_path_1(const char *pattern, const char *refname,
				unsigned flags)
{
	/* XXX: should be size_t's. */
	int plen = strlen(pattern);
	int namelen = strlen(refname);

	if ((plen <= namelen) &&
	    !strncmp(refname, pattern, plen) &&
	    (refname[plen] == '\0' ||
	     refname[plen] == '/' ||
	     pattern[plen-1] == '/'))
		return 1;
	if (!wildmatch(pattern, refname, flags))
		return 1;
	return 0;
}
